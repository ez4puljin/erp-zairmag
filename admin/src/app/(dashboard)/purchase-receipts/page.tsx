...
import { PackagePlus, Plus, ChevronDown, ChevronUp, Search, X, Printer, Package } from 'lucide-react';
...
====================================`;
  ...
  return (
    <div className="space-y-5 animate-ios-fade-in max-w-[1200px]">
      <div className="flex items-center justify-between">
        <h1 className="text-[22px] font-bold text-[#1A1D26]">Нийлүүлэгчээс орлого</h1>
        <button
          onClick={() => { setShowForm(!showForm); if (showForm) resetForm(); }}
          className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-[13px] font-semibold text-white shadow-md shadow-[#10B981]/25 bg-[#10B981] hover:bg-[#059669] transition-colors"
        >
      ...
      {/* Create Form */}
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-[#E8ECF0] p-5 space-y-5">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <label className="block text-[11px] font-bold text-[#8C8FA3] uppercase mb-1.5">Нийлүүлэгч *</label>
            ...
            <div>
              <label className="block text-[11px] font-bold text-[#8C8FA3] uppercase mb-1.5">Огноо *</label>
              <input type="date" value={receivedDate} onChange={e => setReceivedDate(e.target.value)} required className={inputClass} />
            </div>
            <div>
              <label className="block text-[11px] font-bold text-[#8C8FA3] uppercase mb-1.5">Тэмдэглэл</label>
          ...
          <div>
            ...
            <div ref={searchRef} className="relative mb-3">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-[#F5F6FA] border border-[#E8ECF0] text-[14px] outline-none focus:border-[#007AFF] focus:ring-2 focus:ring-[#007AFF]/15"
              />
          ...
          {formItems.length > 0 && (
            <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    <div className="text-center">
                      ...
                      <input
                        ...
                        className="w-16 px-2 py-1.5 rounded-lg bg-white border border-[#E8ECF0] text-[13px] text-center font-bold outline-none focus:border-[#007AFF]"
                      />
                    </div>
                    <div className="text-center">
                      ...
                      <input
                        ...
                        className="w-24 px-2 py-1.5 rounded-lg bg-white border border-[#E8ECF0] text-[13px] text-center font-bold outline-none focus:border-[#007AFF]"
                      />
                    </div>
                    <div className="text-right w-24">
                      <label className="text-[9px] text-[#8C8FA3] block">Нийт</label>
                      <p className="text-[13px] font-bold text-[#1A1D26]">₮{(item.quantity * item.unitPrice).toLocaleString()}</p>
                    </div>
                    <button type="button" onClick={() => removeFormItem(idx)} className="p-1.5 rounded-lg hover:bg-red-50 text-[#EF4444]">
                      <X className="w-4 h-4" />
                    </button>
                  </div>
                </div>
              ...
              {/* Grand total */}
              <div className="flex items-center justify-between px-3 py-3 rounded-xl bg-[#EFF6FF] border border-[#007AFF]/20">
                <span className="text-[13px] font-bold text-[#1A1D26]">Нийт дүн:</span>
                <span className="text-[18px] font-bold text-[#007AFF]">₮{grandTotal.toLocaleString()}</span>
              </div>
            </div>
          )}

          <div className="flex justify-end gap-3">
            <button type="button" onClick={() => { setShowForm(false); resetForm(); }} className="px-5 py-2.5 rounded-xl bg-[#F5F6FA] text-[#8C8FA3] font-semibold text-[13px]">Болих</button>
      ...
      <div className="bg-white rounded-2xl border border-[#E8ECF0] overflow-hidden">
        {loading ? (
          ...
          <div className="py-16 text-center">
            <PackagePlus className="w-12 h-12 text-[#D0D2DA] mx-auto mb-3" />
            <p className="text-[15px] font-semibold text-[#8C8FA3]">Орлого бүртгэл байхгүй</p>
          </div>
        ) : (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[12px] text-[#8C8FA3]">{r.supplier?.name}</span>
                      </div>
                      <p className="text-[12px] text-[#8C8FA3]">
                        {new Date(r.receivedAt).toLocaleDateString('mn-MN')} · {r.items?.length} бараа
                      </p>
                    </div>
                    <span className="text-[15px] font-bold text-[#1A1D26] shrink-0">₮{Number(r.totalAmount).toLocaleString()}</span>
                    {expanded === r.id ? <ChevronUp className="w-4 h-4 text-[#D0D2DA]" /> : <ChevronDown className="w-4 h-4 text-[#D0D2DA]" />}
                  </button>
                  <button
                    onClick={() => printReceipt(r)}
                    className="p-2 rounded-lg hover:bg-[#F5F6FA] text-[#8C8FA3] hover:text-[#007AFF] transition-colors shrink-0"
                    title="Баримт хэвлэх"
                  >
                      ...
                      <tbody>
                        {r.items.map((i: any) => (
                          <tr key={i.id} className="border-b border-[#F2F4F7] last:border-0">
                            ...
                            <td className="py-1.5 text-right">{i.quantity} {UNIT_LABELS[i.product?.unit] || ''}</td>
                            <td className="py-1.5 text-right">₮{Number(i.unitPrice).toLocaleString()}</td>
                            <td className="py-1.5 text-right font-bold">₮{Number(i.lineTotal).toLocaleString()}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
