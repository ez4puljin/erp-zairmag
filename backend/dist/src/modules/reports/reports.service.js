...
let ReportsService = class ReportsService {
    ...
    async getSalesByCategory() {
        ...
        return categories.map((cat) => ({
            ...
            totalSales: cat.customers.reduce((sum, c) => sum + c.orders.reduce((s, o) => s + Number(o.totalAmount), 0), 0),
            thisMonthSales: cat.customers.reduce((sum, c) => sum +
                c.orders
                    .filter((o) => {
    ...
    async getCustomerDebtReport() {
            ...
            select: {
                ...
                _count: {
                    select: {
                        orders: {
                            where: {
                                status: { in: [client_1.OrderStatus.APPROVED, client_1.OrderStatus.SHIPPING] },
                            },
                        },
                    },
                },
        ...
        const totalDebt = customers.reduce((sum, c) => sum + Number(c.outstandingDebt), 0);
        return {
            ...
            customers: customers.map((c) => ({
                ...
                creditUsagePercent: Number(c.creditLimit) > 0
    ...
    async getDriverReport(from, to, driverId) {
        const fromDate = new Date(from);
        const toDate = new Date(to);
        toDate.setHours(23, 59, 59, 999);
        ...
        const addToBreakdown = (b, method, amount) => {
            if (!b[method])
        ...
        const drivers = Array.from(driversMap.values()).sort((a, b) => b.summary.totalRevenue - a.summary.totalRevenue);
        return {
            summary: {
