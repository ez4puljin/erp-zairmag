...
export default function EditExpenseCategoryScreen() {
  ...
  useEffect(() => {
    if (!id) return;
    api.get(`/api/expense-categories/${id}`).then(res => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await api.patch(`/api/expense-categories/${id}`, { name: form.name.trim(), description: form.description.trim() || undefined });
  ...
  const handleDelete = async () => {
    ...
    if (!ok) return;
    try { await api.delete(`/api/expense-categories/${id}`); invalidateListCache('/api/expense-categories'); router.back(); }
